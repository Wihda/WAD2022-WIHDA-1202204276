...
<html lang="en">
  ...
  <body>
    ...
    <?php
        ...
        $query = mysqli_query($conn, "SELECT * FROM showrooms WHERE id_mobil='$id_mobil'") or die("database error:". mysqli_error($conn));
    ...
    <h1>Edit</h1>
    <p style="color:grey">Edit Mobil <?php echo $record['nama_mobil']?></p>

    <div class="container">
    <div class="row">
        <div class="col">
        <img src="../MODUL4 WIHDA/images/<?php echo $record["foto_mobil"];?>" class="rounded" alt="car" style="width: 90%">
    </div>

    <div class="col">
        <form action="{{url('/layouts_m5/edit')}}" method="GET" enctype="multipart/form-data">
        <input type="hidden" name="id_mobil" value="<?php echo $id_mobil; ?>">
        <div class="mb-3">
            <label for="name" class="form-label"><strong>Nama Mobil</strong></label>
                        <input type="text" class="form-control" name="nama_mobil" value="<?php echo $record['nama_mobil'] ?>" >
                    </div>
                    <div class="mb-3">
                        <label for="nama" class="form-label"><strong>Nama Pemilik</strong></label> <br>
                        <input type="text" class="form-control" name="pemilik_mobil" value="<?php echo $record['pemilik_mobil'] ?>" >
                    </div>
                    <div class="mb-3">
                        <label for="merk" class="form-label"><strong>Merk</strong></label> <br>
                        <input type="text" class="form-control" name="merk_mobil" value="<?php echo $record['merk_mobil'] ?>" >
                    </div>
                    <div class="mb-3">
                        <label for="date" class="form-label"><strong>Tanggal Beli</strong></label> <br>
                        <input type="date" class="form-control" name="tanggal_beli" value="<?php echo $record['tanggal_beli'] ?>" >
                    </div>
                    <div class="mb-3">
                        <label for="Deskripsi" class="form-label"><strong>Deskripsi</strong></label>
                        <textarea class="form-control" rows="3" name="deskripsi" ><?php echo $record['deskripsi'] ?></textarea>
                    </div>
                    <div class="mb-3">
                        <label for="file" class="form-label"><strong>Foto</strong></label>
                        <input class="form-control" type="file" name="foto_mobil" value="foto_mobil" placeholder="3">
                    </div>
                    <br>
                    <div class="mb-3">
                    <p><strong>Status Pembayaran</strong></p>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" id="inlineCheckbox1" name="status_pembayaran" value="lunas" <?php echo active_radio_button("lunas", $record['status_pembayaran'])?> >
                        <label class="form-check-label" for="inlineCheckbox1">Lunas</label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" id="inlineCheckbox2" name="status_pembayaran" value="belum lunas"<?php echo active_radio_button("belum lunas", $record['status_pembayaran'])?> >
                        <label class="form-check-label" for="inlineCheckbox2">Belum Lunas</label>
                    </div>
                        <br><br>
                        <button type="submit" class="btn btn-primary btn-sm" style="padding-left:25px; padding-right:25px">Simpan</button>
                    </form>
                    </div>
                </div>
            </div>
